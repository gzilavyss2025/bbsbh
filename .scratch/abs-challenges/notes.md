...
- `scripts/lib/abs/` — the pure half, in two files behind `index.mjs`:
  `rows.mjs` (one feed to rows) and `export.mjs` (rows to
  `public/data/abs-challenges.json`). Unit-tested in
