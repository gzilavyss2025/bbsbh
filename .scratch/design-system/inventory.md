...
and 9, so the table was the stale half. The corrected figure is 32 cards and 10
pills. `.gamecardstack` is the 33rd name in that prose list and is **not a card**
— it is the slate's stack container. It is excluded here and counted in the
"not a card" group below.
...
## The headline: there are not 32 cards

Of 32 card blocks, **13 draw the same box**. Seven draw no box at all. Five are
...
### Group C — namespace only, no base rule (7). Verdict: nothing to merge.
...
| `.pin-card` | `12-sealbox.css` | **nothing — zero consumers** | **delete** |

`.pin-card` is one selector in `12-sealbox.css` with no consumer anywhere in
`src/`. It is dead. Confirm against git history before removing it, in case it is
a hook something outside `src/` sets.

**The naming finding.** Seven blocks are called `…card` and are not cards. That
is why the count looked like 32. A namespace that ends in `card` reads as a card
to the next agent that greps, and it is how the 32nd block gets written. Step 2
should rename these, or step 3's guard will keep counting them.
...
one-purpose typeface, and the largest rule count of any block (178 selector
hits). It is the app's front door. It can consume shared inner parts; its box
should stay its own.
...
That is **four variants covering 17 of the 32 blocks.** Seven more need nothing
