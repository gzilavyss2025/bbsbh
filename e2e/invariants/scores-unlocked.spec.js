...
//
// Note: the innings/box-score-content checks need the anchor game's feed —
// installMockApi (mock-api.js) serves it from a captured fixture, so they no
// longer need to be guarded against a no-feed environment the way they used
// to be.
...
test('with the pass on, a game unseals without a tap — and NEVER writes the reveal mark', async ({
  ...
  // Seed the pass, then reload — a real navigation, not just in-memory
  // state, so it takes effect the same way it would for a returning user.
  // NOT page.addInitScript: that registration outlives this reload and
  // would silently re-write the pass on the "turn it off" reload below too.
  await page.evaluate(
  ...
  // The top of the 1st reads as revealed with no tap and no "Tap to reveal"
  // cover. installMockApi keeps the feed available deterministically (a
  // captured fixture for this anchor game), so this no longer needs to be
  // guarded against a no-feed environment.
  await expect(page.locator('.rolling__pick').first()).toBeVisible()
  ...
  await expect(page.locator('.innings')).toBeVisible()
...
test('with the pass on, the box score unseals too — and still never writes the mark', async ({
  ...
  // Seed the pass, then reload — see the top1 test above for why not
  // page.addInitScript (it would re-write the pass on the reload below too).
  await page.evaluate(
    ([p]) => window.localStorage.setItem(p, String(Date.now() + 24 * 3600 * 1000)),
    [PASS_KEY],
  )
  ...
  await expect(page.locator('.boxscore')).toBeVisible()
