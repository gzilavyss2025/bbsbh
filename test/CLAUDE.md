...
| band-focus-ring.test.js | 19 | src/styles/**, src/tokens/**, src/lib/teams.js, scripts/check-focus-ring.mjs | The focus ring on a coloured band (#1215): paper with an ink halo on each band control, focus and press shown together, one ring colour at 3:1 on every landed club bar and hero tile, and the guard failing an outline with a colour and no style |
...
| boxlines-card-facets.test.js | 16 | src/api/boxlines/cardFacets.js, facets.js | Every door on the player page's Game lines card, checked against the same `facetPlan` the sheet calls — an unknown kind keeps NOTHING, so a typo would otherwise ship as a door that opens on an empty ledger |
| boxlines-facets.test.js | 12 | src/api/boxlines/facets.js | A facet resolves to one `opponentId`/`gameTypes`/`keep` triple, and `keep` can only narrow |
| boxlines-rows.test.js | 33 | src/api/boxlines/rows.js | The Box Lines cutoff gate (ADR-0069) — strictly before the cutoff, Final AND scored, the request plan, the row shape |
| boxlines-score-recovery.test.js | 16 | src/api/boxlines/rows.js, src/api/boxlines/fetch.js | A game played under a schedule row MLB left stuck at Postponed (#1031): the score recovered off the game's own linescore, and the four things that recovery must NOT be able to do — reach past the cutoff, reach a non-Final game, overwrite a real score, or keep a row when the linescore is empty too |
...
| express-lane-runners.test.js | 12 | src/api/expresslane/runners.js | Express Lane's scoring deck over the captured real game — which men are standing on base at a given cursor and whose diamond a steal gets written on: the bases filling through a real rally, the man erased on a fielder's choice who must not be left on first, the cap that keeps a runner's diamond off the bases he reaches LATER, the paperwork row inside a plate appearance that must not reveal it, and a whole-game sweep asserting no base ever holds two men |
| express-lane-eligibility.test.js | 10 | src/api/expresslane/eligibility.js | Whether pitch film can exist for a game at all — the five rules that decide if the Express Lane door is drawn: MLB only (sportIds 11-14 have zero clips), 2016 forward, no All-Star game, started, not postponed (which reports Final with nothing behind it). Built on stub feeds because the repo's trimmed capture predates `gameData.game` and the team `sport` block; the live paths are cited in the header |
| express-lane-runner.test.js | 17 | src/lib/expresslane/runner.js | Express Lane Tier 3 — the staging loop, wholly offline through an injected resolver, downloader, store and clock: queue order with one request in flight, the resume read BEFORE any fetch, the phantom-playId pass-through that would otherwise deadlock, a 403 and a full disk each stopping the job their own way, eviction on a cursor move, and the sweep that comes back for a clip that published late |
| express-lane-staging.test.js | 36 | src/lib/expresslane/staging.js | Express Lane Tier 3 — the queue state machine and THE FILM GATE: paperwork never waits, an empty resolution never waits and never prompts (#1024), a contiguous frontier the cursor cannot jump, the consented escape, block-vs-pause, and the eviction lookbehind that keeps an expanded plate appearance watchable |
...
| milestone-celebrations.test.js | 13 | src/lib/milestoneCelebrations.js | Game Log milestone shelf's one-shot completion-animation store — same shape as prompts.js, one id per collection per level, MLB keeping the bare id |
