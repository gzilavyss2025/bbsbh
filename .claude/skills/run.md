...
`statsapi.mlb.com`. The `npm test` unit suite (232 passing, `test/*.test.js`,
CI-gated) covers the pure data layer, and `e2e/invariants/` has permanent
Playwright specs guarding the spoiler-reveal mechanism — see CLAUDE.md. Neither
substitutes for a browser-level check: for anything user-visible, verification
still means actually loading screens in a browser. This skill exists to cut
the token/time cost of that loop: fixed port, no port-discovery, no manual
server start/stop/poll cycle, and a pinned set of real games with known-rare
events so you're not hunting for a live game each session.
...
`npm run e2e` is a verification harness, not CI-gated — the CI-gated suite is
the `npm test` unit suite (see CLAUDE.md).
...
modal never blocks the first-visit slate. Keep the server running when done —
AGENTS.md/CLAUDE.md require leaving it up for the maintainer's handoff, with a
clickable local URL in the final summary. Don't kill it unless the port is
needed for another task in this same session.
