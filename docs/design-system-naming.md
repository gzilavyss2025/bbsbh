...
`.card` and `.gamecard` (card; `.card` is the Card component's, which took over `.thub-card` in #1221), `.btn` (btn), `.sheet` (sheet, in
...
| `.cmdmap__chip` | 5 | `.cmdmap__btn` | #1131 | `components/charts/CommandMap.jsx`, `components/charts/GloveTarget.jsx`, `src/index.css` †, `styles/26d-command-map.css`, `styles/26f-glove-target.css` † | moved onto `<Pill role="control">` in slice 4 of #1131, which kept this name as the host's hook: the rename to the target is still open. |
...
| `.slate-filterbar__chip` | 5 | `.slate-filterbar__btn` | #1131 | `screens/GameSelect.jsx`, `styles/22-box-score-tables.css` | moved onto `<Pill role="control">` in slice 4 of #1131, which kept this name as the host's hook: the rename to the target is still open. |
...
| `.slate-filterbar__chip--active` | 4, 5 | `.slate-filterbar__btn.is-active` | #1131 | `screens/GameSelect.jsx`, `styles/22-box-score-tables.css` | landed in slice 4 of #1131: the class is DELETED, not renamed. The selected state is `aria-pressed` now (#1166 point 4), so no class carries it, and the target's `.is-*` is not needed. |
| `.cmdmap__chip--on` | 4, 5 | `.cmdmap__btn.is-on` | #1131 | `components/charts/CommandMap.jsx`, `components/charts/GloveTarget.jsx`, `styles/26d-command-map.css` | landed in slice 4 of #1131: the class is DELETED, not renamed. The selected state is `aria-pressed` now (#1166 point 4), so no class carries it, and the target's `.is-*` is not needed. |
...
| `.chalcard` | 1, 2 | `.chal` | #1113 | `screens/designlab/blocks.jsx`, `screens/designlab/catalog.js`, `screens/team/modules/TeamChallengeCard.jsx`, `styles/report/challenge-card.css` | landed in #1221, with the card: `thub-card chalcard` is `<Card className="chal">`, and every `.chalcard__*` part is `.chal__*`. |
