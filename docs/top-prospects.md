...
weekly via the Monday-only weekly block in
`.github/workflows/update-nightly-data.yml` (its own `update-top-prospects.yml`
cron until 2026-08-28), which commits the refreshed file straight to `main` in
the nightly batch's single commit — that push rides the existing Vercel
