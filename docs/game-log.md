...
### The seventeen files display copy lives in
...
| Stamp sheet | pane titles `Every club` / `Every ballpark`, ledes *“Stamp a game against every club at this level — either dugout counts.”* / *“Stamp a game AT each park — the home club is the park.”* · level toggle `MLB` `AAA` `AA` `A+` `A` · an unposted level says *“This level’s clubs are not posted yet.”* **The `n of 30` line, the completed-set ring and the completion beat are all one prop (`counts`) and belong to `/logbook/stats` alone** — the book page renders the identical panes without it, which is how the clubs grid above keeps its rule while sharing the art |
