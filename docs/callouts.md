...
| Between Innings (focus mode's post-half hold) | `buildBetweenInnings` (`src/api/between-innings.js`) via `BetweenInnings` | Same entering-tonight families as Margin Notes/the pre-half strip, through a stricter hard allowlist that drops every checkpoint family that reads tonight's score |
