...
`gen-team-score.mjs` and `gen-season-score.mjs` now write into a shared
...
directly with no `npm install` step; a built-in avoids adding install
latency and avoids native-binary platform risk.
...
**Dumps are split one file per table-group, not one shared file**, so a
future table written by a generator on its own independently scheduled cron
can never collide with another group's: a single dump fully rewritten on
every run would let whichever workflow pushes second silently overwrite the
other's table with a stale copy. Splitting restores the same per-file
isolation the all-JSON setup had.
...
generator (its own cron, fetching each newly-Final game's completed feed and
replaying it through the scoring functions). Deferred as its own decision,
not started here.
