...
- `docs/callouts.md` is the catalog: every callout family, its trigger,
  surface, gate, and worthiness score (the 0–100 ranking the box score's
  Insights card and the pre-half strip sort/cap by). ADR-0014 is the tense
  rule — play cards fold in revealed plays only ("that's No. 16 this
  season"); result-aware wording ("moved to 18-2") lives solely in the
  box-score roll-up. Read both before adding or rewording a note.
...
- The precompute covers **MLB and the four full-season MiLB levels** (every
  person-stats fetch for a MiLB player must carry his level's `sportId` or the
  API silently returns his empty MLB line); career-derived families and the
  standings splits stay MLB-only. The per-date files are ~1 MB, kept out of
  the PWA precache (`vite.config.js`).
- Before adding a new data source, check whether an existing split file covers
  it: `vs-team-splits` (career vs opponent), the API's own `statSplits`
  (RISP, vs-L/vs-R, and the sah/sbh/sti leverage buckets), and per-PA
  `playLog` (times-through-the-order) are already wired in and reusable.
