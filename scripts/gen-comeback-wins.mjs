...
// WIN counts: a win in which the team's win probability dropped below 10 / 20 /
// 30% at some point in the game. Buckets are NESTED (a sub-10 win also counts
// sub-20 and sub-30, so sub10 <= sub20 <= sub30). Shown on the Team Page as a
// ranked "Comeback wins" card when non-zero — see src/api/comebackWins.js.
//
...
// history, takes the WINNER's minimum win % over the game, buckets it, and folds
// it into the running per-team totals (SQLite, docs/adr/0021). A Final game's
// win-prob history never changes, so an already-ingested game is never
// refetched (guarded by comeback_ingested_games). MLB only (sportId 1) — the
// winProbability endpoint is MLB-only (see src/api/game.js) and a league rank
// needs the whole 30-team pool anyway; MiLB parks have no endpoint.
//
// Run by hand:
...
// The winner's MINIMUM win probability across the whole game. Home's share is
// homeTeamWinProbability directly; the away team's is 100 − that, so the away
// team's minimum is 100 − the home team's MAXIMUM. Null when the endpoint
// carries no numeric win prob (shouldn't happen for a Final MLB game, but guard).
export function winnerMinWinProb(winProb, winnerIsHome) {
  ...
  if (!seen) return null
  return winnerIsHome ? minHome : 100 - maxHome
}
...
async function main() {
  ...
  const upsert = db.prepare(
    `INSERT INTO comeback_win_totals (team_id, season, wins, sub10, sub20, sub30)
     VALUES (?, ?, 1, ?, ?, ?)
  ...
  for (const dateStr of dates) {
    ...
    for (const g of (slate.dates ?? []).flatMap((d) => d.games ?? [])) {
      ...
      candidates.push({ gamePk: g.gamePk, season: Number(dateStr.slice(0, 4)), winnerId, winnerIsHome })
    }
  }
  ...
  for (const c of candidates) {
    try {
      ...
      const b = comebackBuckets(winnerMinWinProb(wp, c.winnerIsHome))
      upsert.run(c.winnerId, c.season, b.sub10, b.sub20, b.sub30)
