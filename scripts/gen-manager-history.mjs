// Regenerates public/data/manager-history/{NN}.json — every current MLB team's
...
// generator and the reader cannot disagree about where a man's record went).
// The page shows one person and a record is ~280 bytes; the league-wide file
// this replaced was 1.1 MB, 45% of which was pretty-print whitespace. These
// shards are also this job's own merge base in --current-only mode.
const outDir = join(here, '..', 'public', 'data', 'manager-history')
...
// Rides on every shard, since any one of them may be the file someone opens.
const HINT =
  'GENERATED by scripts/gen-manager-history.mjs, bucketed on personId % 100. Full backfill: ' +
  '`node scripts/gen-manager-history.mjs` (seasons 2000-present, all 30 teams, rebuilds from ' +
  'scratch). Nightly-cron-shaped incremental: `node scripts/gen-manager-history.mjs --current-only` ' +
  '(this season only, merges into the existing shards). A mid-season managerial change\'s per-stint ' +
  'W/L split is resolved via scripts/manager-transitions-seed.json (edit the SEED, never this file); ' +
...
async function main() {
  ...
  // One file per bucket. `_hint` rides on each shard: whichever one someone
  // opens first should say where it came from and where to edit instead.
  const generatedAt = new Date().toISOString()
  const coverage = CURRENT_ONLY
    ? { seasons: [season, season], mode: 'current-only' }
    : { seasons: [START_YEAR, season], mode: 'backfill' }
  const buckets = new Map()
  for (const [personId, stints] of Object.entries(sortedOut)) {
    const key = shardKey100(personId)
    if (!buckets.has(key)) buckets.set(key, {})
    buckets.get(key)[personId] = stints
  }
  const { written } = await writeShards(
    outDir,
    [...buckets].map(([key, byPersonId]) => [key, { _hint: HINT, generatedAt, coverage, byPersonId }]),
  )

  console.log(
    `wrote ${written} shards to ${outDir}: ${Object.keys(sortedOut).length} people, ` +
      `${mergedNeedsResearch.length} team-seasons needing research`,
  )
}
