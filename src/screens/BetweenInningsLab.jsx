...
import { HalfTally } from '../components/gamehud/HalfTally.jsx'
...
// game feed, no score/reveal content.
...
// A card demo's marginNotes[0] is always excluded (it's treated as
// whatever HalfTally already promoted as its one-liner) — a filler entry
// keeps the note actually under test from being the one that gets dropped.
const filler = { text: 'filler', kind: 'homeAway', score: 1, dedupeKey: 'filler' }
const cycleNotes = [
  headshotNote,
  ...
  return (
    <div className="screen">
      ...
      <p className="hint hint--prose">
        The post-half hold&rsquo;s one-liner and card set, against synthetic fixtures. Unlisted —
        not linked from anywhere else in the app.
      </p>

      <section className="bilab__entry">
        <h2 className="bilab__title">One-liner — a note exists</h2>
        <HalfTally feed={feed()} inning={1} half="top" battingSide="away" getDerived={() => ({})} topNote={headshotNote.text} />
      </section>

      <section className="bilab__entry">
        <h2 className="bilab__title">One-liner — nothing to promote yet</h2>
        <HalfTally feed={feed()} inning={1} half="top" battingSide="away" getDerived={() => ({})} topNote={null} />
      </section>

      <section className="bilab__entry">
        <h2 className="bilab__title">Card — person-scoped fact (headshot)</h2>
        <BetweenInnings feed={feed()} bundle={minimalBundle} marginNotes={[filler, headshotNote]} inning={1} half="top" revealedThrough={0} gameDate={GAME_DATE} />
      </section>

      <section className="bilab__entry">
        <h2 className="bilab__title">Card — team-level fact (no avatar slot)</h2>
        <BetweenInnings feed={feed({ probables: false })} bundle={{ away: bundle.away, teamRecords: { away: bundle.teamRecords.away } }} marginNotes={[]} inning={1} half="top" revealedThrough={0} gameDate={GAME_DATE} />
      </section>

      <section className="bilab__entry">
        <h2 className="bilab__title">Card — missing photo (fallback chain: photo → logo → monogram)</h2>
        <BetweenInnings feed={feed()} bundle={minimalBundle} marginNotes={[filler, fallbackNote]} inning={1} half="top" revealedThrough={0} gameDate={GAME_DATE} />
      </section>

      <section className="bilab__entry">
        <h2 className="bilab__title">Tap-through — wraps to the first card past the last</h2>
        <BetweenInnings feed={feed({ probables: false })} bundle={minimalBundle} marginNotes={cycleNotes} inning={1} half="top" revealedThrough={0} gameDate={GAME_DATE} />
      </section>

      <section className="bilab__entry">
        <h2 className="bilab__title">Scenario — quiet half (top 6, 1-2-3 inning)</h2>
        <div className="bilab__group">
          <HalfTally
            feed={feed()} inning={6} half="top" battingSide="away" getDerived={() => ({})}
            topNote="Peralta is riding a 3-outing scoreless streak entering tonight."
          />
          <BetweenInnings
      ...
      <section className="bilab__entry">
        ...
        <div className="bilab__group">
          <HalfTally feed={feed()} inning={4} half="bottom" battingSide="home" getDerived={() => ({})} topNote={headshotNote.text} />
          <BetweenInnings feed={feed()} bundle={bundle} marginNotes={cycleNotes} inning={4} half="bottom" revealedThrough={halfIndex(4, 'bottom')} gameDate={GAME_DATE} />
        </div>
      </section>
      ...
      <section className="bilab__entry">
        ...
        <div className="bilab__group">
          <HalfTally feed={feed()} inning={1} half="top" battingSide="away" getDerived={() => ({})} topNote={null} />
          <BetweenInnings feed={feed()} bundle={bundle} marginNotes={[]} inning={1} half="top" revealedThrough={halfIndex(1, 'top')} gameDate={GAME_DATE} />
        </div>
      </section>
    </div>
  )
