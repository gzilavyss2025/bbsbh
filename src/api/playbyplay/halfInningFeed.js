  ...
  for (const play of plays) {
    ...
    for (const [evIndex, e] of (play.playEvents ?? []).entries()) {
      ...
      if (STOPPAGE_EVENTS.has(et)) {
        ...
        // nextStepBoundary and focusWindows (a placement is neither a reveal
        // tap nor a display window of its own — it bundles forward with the
        // leadoff plate appearance, exactly as today's event note does, so no
        // already-stored step count changes meaning) and PlayByPlay's
        // `hasAtBat` (a live half whose only fetched content is the placement
        // must not read as a whole revealed half).
        //
