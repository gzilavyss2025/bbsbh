...
// docs/data-enrichment.md §5). `rosters` carries every named selectee,
...
// selections, not a boxscore scan, so he still shows. `games` is
// season -> gamePk only; the screen resolves each game's live team/date info
// via fetchGameCardsByPk (schedule.js), same as TopGamesPage, so a franchise
// rename never goes stale in this file.
//
...
// Degrades to an empty list before the file exists or on any failure.
// Cached in-memory for the session since the file only changes on a
// hand-run regenerate.
let cached = null
...
export async function loadAllStarRosters() {
  ...
  try {
    ...
    cached = { seasons: [], rosters: {}, games: {}, generatedAt: null }
  }
  return cached
}
