...
| `playerstats/` | `RecentFormCard`, `SplitsVsTeam`, `FoulCard`, `MilestoneWatchCard`, `PitcherWorkloadCard`, `SprayMapSection` | Player statistical cards (as distinct from `charts/`'s plotted quantities). `SprayMapSection` is the odd one: a self-fetching MOUNT for a `charts/` card, so the player page carries one line and the whole block — title, fetch and every reason to render nothing — moves together |
