...
| `transactions/` | `TeamTransactionsCard`, `TradeCard`, `TransactionTimeline`, `LeagueMovesCard` | Roster-move surfaces. The last is the home slate's 48-hour card (issue #772, `api/transactions/leagueFeed.js`), and it deliberately does NOT reuse `TeamTransactionsCard`: that deck sits under a club's own name, so `stripLeadingClub` takes the club out of every cutline — drop those cards on a league-wide feed and half read "Sent LHP Kent Emanuel outright to Louisville Bats" with no subject at all. Naming the club is the sentence's missing half here, which is why each row leads with a colour spine, a mark and an abbreviation. It is a ledger rather than a deck because a real 48 hours holds ~35 stories and the worst of a measured month held 125 — the wrong shape for a swipe on the busiest page in the app. It fits itself to the space it has and ends on a whole row; `e2e/league-moves-card.spec.js` measures that, because no unit test can |
