...
// Team colors
//
// Hand-maintained primary brand color per MLB club, decorative use only (e.g.
// tinting a home game on the team page's schedule calendar). There is no live
// source for this: /teams/{id} carries no color field, and the logo CDN serves
// only SVG marks, never a palette (see docs/uniforms-and-logos.md). MLB only —
// MiLB affiliates have no entry here and callers should fall back to a neutral
// tone.
export const TEAM_PRIMARY_COLOR = {
  108: '#BA0021', // Angels
...
export function teamPrimaryColor(teamId) {
  return TEAM_PRIMARY_COLOR[teamId] ?? null
}
