...
// spreadsheets were not consistent across the decades they cover. A Map, not
// a plain object literal: on an object literal, a lookup like
// obj['constructor'] silently returns Object's own constructor function
// instead of undefined, which a careless caller could read as a truthy
// "teamId". A Map has no such inherited keys.
const CODE_TO_TEAM_ID = new Map([
...
for (const [code, teamId] of CODE_TO_TEAM_ID) {
...
// it is a confirmed typo (see file header), not a fourth Asian league. Keys
// are uppercase because lookups below normalize to uppercase first — the raw
// CSV casing (dnp, retired lowercase; KBO/MEX/NPB/NBP uppercase) does not
// need two different code paths.
const CODE_TO_DESTINATION = new Map([
  ['KBO', 'kbo'],
  ['NPB', 'npb'],
...
// The shared return value for a blank cell — a known, legitimate CSV state
// (free_agency.csv leaves old_club blank for an undrafted international
// signing's first deal, and new_club blank while a free agent's move is
// still pending), never mutated, so one frozen instance is fine to hand back
// on every call.
const BLANK = Object.freeze({ teamId: null, franchiseId: null, destination: null, blank: true })

// Resolves one raw club-code cell from arbitration.csv's club, extensions.csv's
// club, or free_agency.csv's old_club/new_club. Exactly one of four
// distinguishable outcomes:
//
//   { teamId: 120, franchiseId: 120, destination: null, blank: false }
//     - a known MLB club
//   { teamId: null, franchiseId: null, destination: 'npb', blank: false }
//     - left MLB for a named destination
//   { teamId: null, franchiseId: null, destination: null, blank: true }
//     - a blank cell: a known, legitimate absence, not a data defect
//   null
//     - a code this table has never seen: a data DEFECT someone must see,
//       fed by a source export that changed shape. Never thrown, never
//       guessed at.
//
// A blank cell and an unrecognized code are different facts and must stay
// distinguishable: result === null is the defect, result.blank === true is
// the known absence. Collapsing both to plain null (an earlier version of
...
// separate franchise concept to carry. It is still a distinct field so a
// caller who means "this franchise, whichever club currently wears it" can
...
export function resolveClubCode(code, _season) {
  ...
  if (CODE_TO_TEAM_ID.has(upper)) {
    const teamId = CODE_TO_TEAM_ID.get(upper)
    return { teamId, franchiseId: teamId, destination: null, blank: false }
  }

  if (CODE_TO_DESTINATION.has(upper)) {
    return { teamId: null, franchiseId: null, destination: CODE_TO_DESTINATION.get(upper), blank: false }
  }
