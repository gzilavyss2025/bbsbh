...
export const starDiversityEntry = {
  ...
  technical: [
    'top1Share/top2Share/hhi: computed over each team-season\'s players with POSITIVE credited WAR only, split hitting vs. pitching. hhi is a Herfindahl-style index (sum of each positive-WAR player\'s share of team positive WAR, squared). Credited WAR comes from public/data/war-history (FanGraphs season totals, personId-keyed) joined against roster-age-cache.json (spike #1\'s teamId-filtered roster/playing-time cache); a player who split a season between two teams has his war-history WAR prorated by playing-time share at each stop, since war-history carries no team attribution at all (new trap, standingNotes.js: traded-player-war-has-no-team-split).',
