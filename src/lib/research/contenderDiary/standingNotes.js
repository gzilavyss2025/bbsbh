...
export const TRAPS = [
  ...
  {
    id: 'one-sided-baseline-measures-the-opposition',
    title: 'Holding a player to "his own regular-season rates" in October measures who he FACED, not the month',
    body: 'It is the most natural test in the world: give each October hitter the plate appearances October really gave him, let him hit at his own regular-season rate, and see how far short he came. Do that and October hitters land 88 points of OPS below themselves, in 24 of 25 seasons. Then run the identical test from the mound — every October pitcher at his own rate, given his real October batters — and the pitchers come up 37 points short too. Both sides cannot be having a bad month in the same game. What each one-sided test actually measures is that the opposition is better than league average, which is true of BOTH dugouts in October and is not a finding about the month at all. The fix is to hold both ends of the matchup at once (additively for OPS, by odds ratio for a per-plate-appearance rate); the residual drops from 88 points to 14, and the October strikeout jump disappears entirely. Any future spike that compares postseason to regular-season PERFORMANCE has to combine both sides, and a one-sided number must never be quoted on its own.',
  },
  {
    id: 'a-maximum-is-biased-upward-in-a-small-sample',
    title: 'Any statistic that is a MAXIMUM runs high in October just because October is short',
    body: 'A postseason sample is roughly a tenth the size of a regular season. Averages survive that fine. Maximums do not: the largest of a handful of noisy shares is biased upward when the sample shrinks, so a pitcher who changed nothing at all would still appear to lean harder on his best pitch in October. The same applies to any "top-N concentration," longest-streak, or squared-share measure. The control that works is a shrunken baseline — draw the same number of events October gave him, at random, from his own regular-season distribution, and measure that identically. For the pitch-mix spike about a quarter of the apparent narrowing was pure sample size; the rest was real, but the uncorrected number would have overclaimed by a third.',
  },
  {
    id: 'check-the-database-before-re-deriving',
    title: 'The join a new spike needs may already exist as a view',
    body: 'scripts/research-db.mjs registers every cached research JSON panel from both diaries — factor panels here and prospect-research panels alike — as a free, read-only DuckDB view over the JSON, queryable with plain SQL. Before a spike re-derives a join across factor panels, or re-pulls statsapi for a panel that might already exist, check the catalog in docs/agents/research-database.md. The panel this spike needs may already be sitting there.',
  },
  {
    id: 'a-null-needs-a-stated-alternative',
    title: 'A finding of "nothing there" is worthless until you say what SOMETHING would have looked like',
    body: 'This program keeps producing nulls, and one of them was being read completely backwards. "The better club won only half its postseason series, so October is a coin" sounds airtight — until you build the other world and check. Shrink every club\'s record toward the middle (a season is a noisy way to measure a team), work out each matchup, and run every series in its real format: in a world where the better club genuinely IS better, the better record still only wins about fifty-six times in a hundred. Fifty and fifty-six are not far apart, and a hundred and ninety-eight series cannot tell them apart. The same check reversed a second claim in the same entry: the best record in baseball has won six of twenty-six World Series, which reads as chaos until you notice a pure coin would deliver about three. Any null in this program needs the same treatment — say what the effect WOULD look like, and say how often a pile of data this size would even spot it. Otherwise the finding is about the sample, not about baseball.',
  },
]
